...
fetch(url_cat)
  ...
  .then((data_cat) => {
    ...
    for (let i = 0; i < data_cat.photos.photo.length; i++) {
      const photo_cat = data_cat.photos.photo[i];
      const photoText_cat = getFlickrText(photo_cat);
      // 空の<div>を作る
      ...
      $div.append(
        $('<a>', {
          ...
          'data-text': photoText_cat,
          href: getFlickrPageURL(photo_cat),
          rel: 'noopener noreferrer',
          target: '_blank', // リンクを新規タブで開く
        }).append(
          $('<img>', {
            class: 'img-fluid',
            src: getFlickrImageURL(photo_cat, 'q'),
            alt: photoText_cat,
            width: 150,
            height: 150,
          }),
      ...
      // $div_iをclass='image-gallery animated'に追加する
...
fetch(url_dog)
  ...
  .then((data_dog) => {
    ...
    for (let i = 0; i < data_dog.photos.photo.length; i++) {
      const photo_dog = data_dog.photos.photo[i];
      const photoText_dog = getFlickrText(photo_dog);
      // 空の<div>を作る
      const $div = $('<div>', {
        class: 'image-gallery__item'
      });
      ...
      $div.append(
        $('<a>', {
          class: 'd-inline-block img-tooltip',
          'data-text': photoText_dog,
          href: getFlickrPageURL(photo_dog),
          rel: 'noopener noreferrer',
          target: '_blank', // リンクを新規タブで開く
        }).append(
          $('<img>', {
            class: 'img-fluid',
            src: getFlickrImageURL(photo_dog, 'q'),
            alt: photoText_dog,
            width: 150,
            height: 150,
          }),
    ...
    // $divをclass='image-gallery animated'に追加する
    $div.appendTo('.image-gallery animated');
  }).catch((error) => {
    console.error(`エラーが発生しました： ${error.message}`);
  });
